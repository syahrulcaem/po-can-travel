    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mb-8">
                <h1 class="text-2xl font-bold text-gray-900 mb-2">Edit Review untuk {{ $bus->name }}</h1>
                <p class="text-gray-600">{{ $bus->plate_number }} • {{ $bus->capacity }} kursi</p>
            </div>

            <!-- Review Form -->
            <div class="bg-white rounded-lg shadow-md p-6">
                <form method="POST" action="{{ route('reviews.update', $bus) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="comment" id="comment" rows="4"
                            ...
                            placeholder="Ceritakan pengalaman Anda menggunakan bus ini...">{{ old('comment', $review->comment) }}</textarea>
                        @error('comment')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
